 ...
 * @author HU 
 */
class ControllerSubSalesmanCommissionsApply extends Controller {
	private $error = array();
	
	public function index() {
		$this->load->language('sub_salesman/commissions_apply');
	
		$this->document->setTitle($this->language->get('heading_title'));
	
		$this->load->model('sub_salesman/commissions_apply');
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$this->model_sub_salesman_commissions_apply->edit($this->request->post);
			...
			$this->response->redirect($this->url->link('sub_salesman/commissions_apply', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sub_salesman/commissions_apply', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$apply_total = $this->model_sub_salesman_commissions_apply->getTotalApplys($filter_data);
		
		$results = $this->model_sub_salesman_commissions_apply->getCommissionApplys($filter_data);
		...
		foreach ($results as $result) {
			$data['applys'][] = array(
				...
				'edit'                   => $this->url->link('sub_salesman/commissions_apply/edit', 'token=' . $this->session->data['token'] . '&apply_id=' . $result['apply_id'] . $url, 'SSL'),
			);
		}
		...
		$data['sort_apply_id'] = $this->url->link('sub_salesman/commissions_apply', 'token=' . $this->session->data['token'] . '&sort=apply_id' . $url, 'SSL');
		$data['sort_period_from'] = $this->url->link('sub_salesman/commissions_apply', 'token=' . $this->session->data['token'] . '&sort=period_form' . $url, 'SSL');
		$data['sort_commission_total'] = $this->url->link('sub_salesman/commissions_apply', 'token=' . $this->session->data['token'] . '&sort=commission_total' . $url, 'SSL');
		$data['sort_apply_date'] = $this->url->link('sub_salesman/commissions_apply', 'token=' . $this->session->data['token'] . '&sort=apply_date' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('sub_salesman/commissions_apply', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		$data['sort_payment_status'] = $this->url->link('sub_salesman/commission_totals_apply', 'token=' . $this->session->data['token'] . '&sort=payment_status' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('sub_salesman/commissions_apply', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('sub_salesman/commissions_apply.tpl', $data));
	}
	...
	protected function getFrom() {
		...
		if (isset($this->request->get['apply_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$application = $this->model_sub_salesman_commissions_apply->getCommissionApply($this->request->get['apply_id']);
		} else {
			$application = '';
		}
		...
		$data['action'] = $this->url->link('sub_salesman/commissions_apply/edit', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['cancel'] = $this->url->link('sub_salesman/commissions_apply', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->response->setOutput($this->load->view('sub_salesman/commissions_apply_form.tpl', $data));
	}
