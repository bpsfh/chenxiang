<?php
class ControllerSalesmanContact extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('salesman/contact');
		...
		$this->load->model('salesman/contact');

		$contacts_total = $this->model_salesman_contact->getTotalContacts($filter_data);

		$contacts = $this->model_salesman_contact->getContacts($filter_data);
		...
		$data['action'] = $this->url->link('salesman/contact');
		...
		$data['sort_contact_title'] = $this->url->link('salesman/contact', 'token=' . $this->session->data['token'] . '&sort=contact_title' . $url, 'SSL');
		$data['sort_reply_flg'] = $this->url->link('salesman/contact', 'token=' . $this->session->data['token'] . '&sort=reply_flg' . $url, 'SSL');
		$data['sort_date_contacted'] = $this->url->link('salesman/contact', 'token=' . $this->session->data['token'] . '&sort=date_contacted' . $url, 'SSL');
		$data['sort_date_replied'] = $this->url->link('salesman/contact', 'token=' . $this->session->data['token'] . '&sort=date_replied' . $url, 'SSL');
		...
		$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('salesman/contact', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$data['add'] = $this->url->link('salesman/contact/add', 'token=' . $this->session->data['token'] . $url, 'SSL');

		$pagination = new Pagination();
		$pagination->total = $contacts_total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('salesman/contact', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('salesman/contact.tpl', $data));
	}
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_salesman_contact->addContact($this->request->post);
			...
			$this->response->redirect($this->url->link('salesman/contact', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function validate() {
		if ((utf8_strlen($this->request->post['contact_title']) < 1) || (utf8_strlen($this->request->post['contact_title']) > 32)) {
			$this->error['contact_title'] = $this->language->get('error_contact_title');
		}

		if (!empty($this->request->post['contact_email']) && !preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $this->request->post['contact_email'])) {
			$this->error['contact_email'] = $this->language->get('error_contact_email');
		}

		if (!empty($this->request->post['contact_phone']) && !preg_match('/1[123456789]{1}\d{9}$/', $this->request->post['contact_phone'])) {
			$this->error['contact_phone'] = $this->language->get('error_contact_phone');
		}

		if ((utf8_strlen($this->request->post['contact_content']) < 10) || (utf8_strlen($this->request->post['contact_content']) > 250)) {
			$this->error['contact_content'] = $this->language->get('error_contact_content');
		}
